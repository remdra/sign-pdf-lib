...
export async function generatePdf17Async({ pageCount }: { pageCount?: number } = {}): Promise<Buffer> {
    return await generatePdfWithPdfLibAsync( { pageCount, useObjectStreams: false });
}

export async function generatePdf17StreamsAsync({ pageCount }: { pageCount?: number } = {}): Promise<Buffer> {
    return await generatePdfWithPdfLibAsync( { pageCount, useObjectStreams: true });
}

async function generatePdfWithPdfLibAsync({ pageCount, useObjectStreams }: { pageCount?: number; useObjectStreams?: boolean } = {}): Promise<Buffer> {
...
async function generatePdfWithPdfMakeAsync({ pageCount }: { pageCount?: number } = {}): Promise<Buffer> {
    const fonts = {
        Helvetica: {
            normal: 'Helvetica',
            bold: 'Helvetica-Bold',
...
export async function generateSignedFieldPdfAsync(fieldPdf: Buffer): Promise<Buffer> {
...
export async function generateTamperedPdfAsync(signedPdf: Buffer): Promise<Buffer> {
    return bufferReplace(signedPdf, 'signer@semnezonline.ro', 'xxxxxx@xxxxxxxxxxxx.xx');
}

export async function generateOnlyFirstTamperedPdfAsync(tamperedSignedPdf: Buffer): Promise<Buffer> {
    return await generateSignedPdfAsync(tamperedSignedPdf, { name: 'Signature2' });
}

export async function generateAppendTamperedPdfAsync(signedPdf: Buffer): Promise<Buffer> {
